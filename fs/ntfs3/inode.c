				       ...
				       enum get_block_ctx ctx)
{
	...
	if (ctx == GET_BLOCK_DIRECT_IO_W || ctx == GET_BLOCK_DIRECT_IO_R) {
		ntfs_static_assert(sizeof(size_t) < sizeof(loff_t));
			  ...
			  struct list_head *pages, unsigned nr_pages)
{
	...
	if (is_compressed(ni)) {
		/* For compressed inodes, skip readahead past valid size */
		valid = ni->i_valid;
		if (valid < i_size_read(inode)) {
			/* first page start position from the page list */
			struct page *last_page = list_entry(pages->prev, struct page, lru);
			pos = (loff_t)last_page->index << PAGE_SHIFT;
			if (valid < pos) {
				/* nothing valid to read in this range */
				return 0;
			}
		}
	}
				      ...
				      int buflen)
{
	...
	ntfs_static_assert(!offsetof(struct REPARSE_DATA_BUFFER, ReparseTag));
	ntfs_static_assert(sizeof(u32) == sizeof(rp->ReparseTag));
