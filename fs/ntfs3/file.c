...
static ssize_t ntfs_compress_write(struct kiocb *iocb, struct iov_iter *from)
{
	...
	while (count) {
		...
		for (;;) {
			...
			page = pages[ip];
			cp = copy_page_from_iter(page, off,
							min(tail, bytes), from);
			flush_dcache_page(page);
		...
		__u64 start, __u64 len)
{
	...
	err = fiemap_check_flags(fieinfo, ~FIEMAP_FLAG_XATTR & FIEMAP_FLAGS_COMPAT);
	if (err)
		return err;
	if (len > i_size_read(inode) - start)
		len = i_size_read(inode) - start;
