...
static int ntfs_parse_options(char *options, struct ntfs_mount_options *opts);
...
/* Note: fs_context API removed for kernel 4.20 compatibility */
...
/*
 * ntfs_mount - Legacy mount function for kernel 4.20
 */
...
static int ntfs_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	err = ntfs_parse_options(data, sbi->options);
	if (err < 0)
		return err;
...
/*
 * ntfs_parse_options - Parse mount options for kernel 4.20
 */
/*
 * ntfs_parse_options - Parse mount options for kernel 4.20
 * Match table for mount options
 */
enum {
	Opt_uid, Opt_gid, Opt_umask, Opt_dmask, Opt_fmask,
	Opt_immutable, Opt_discard, Opt_force, Opt_sparse,
	Opt_nohidden, Opt_acl, Opt_showmeta, Opt_prealloc,
	Opt_noacsrules, Opt_iocharset, Opt_ignore, Opt_err,
};

static const match_table_t ntfs_fs_tokens = {
	{Opt_uid, "uid=%u"},
	{Opt_gid, "gid=%u"},
	{Opt_umask, "umask=%o"},
	{Opt_dmask, "dmask=%o"},
...
static int ntfs_parse_options(char *options, struct ntfs_mount_options *opts)
{
	...
	while ((p = strsep(&options, ",")) != NULL) {
		...
		token = match_token(p, ntfs_fs_tokens, args);
		switch (token) {
		case Opt_uid:
		...
		case Opt_iocharset:
			kfree(opts->nls_name);
			opts->nls_name = match_strdup(args);
			break;
		default:
			/* Unknown option - continue parsing */
			break;
		}
	}
	return 1;
